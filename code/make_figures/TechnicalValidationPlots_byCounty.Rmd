...
# take out row crop- close grown crop
...
# colors
cols <- cols <- c("#69ef7b", "#2c457d", "#edb1ff", "#6c36a3", "#4cccb7", "#224f3e", "#bbe272", "#84241a", "#e9bf98", "#378811", "#8bc0dd", "#5d4030", "#f8d147")

set.seed(9) # 7 is OK
cols <- sample(cols, size=length(cols), replace=F)


# cols <- c("#b4ddd4", "#20502e", "#40cf9b", "#D3D3D3", "#808080", "#10558a", "#7cce1e", "#834cd0", "#c7acf9", "#932764", "#38b5fc", "#ea2db9", "#09f54c", "#eb1138", "#80995a")

cols <- c("#aee39a", "#26496d", "#4cf185", "#D3D3D3", "#77c4d3", "#1c5e39", "#bce333", "#572b9e", "#8699ee", "#9f0b64", "#f597fa", "#6c3357", "#6e9d50")


# try on same graph
var.labs <- c('Number of mismatched cells', 'Percent mismatched cells')
...
pct_map_jenks <- ggplot() + geom_sf(data=toplot, aes(fill=population.natural), color=NA) +  
  guides(fill=guide_legend(title='Percent NVC agriculture\nmismatched with CDL')) +
  theme_classic() + 
  scale_fill_viridis(discrete=T) +
  facet_wrap(~CDLYear, ncol=2)
pct_map_jenks
ggplot2::ggsave(plot=pct_map_jenks, './figures/TechnicalValidation_current/MismatchedPixels_ByCounty_2012_to_2021.pdf', width=10, height=12)
...
ggplot2::ggsave(plot=seventeen, paste0('./figures/TechnicalValidation_current/CombinedFigMap_MismatchedPixels_', CDLYear, 'Only.pdf'),
                width=10, height=11)
...
nodata_map_jenks <- readRDS(paste0('./nodata_map_jenks_CDL', CDLYear, '.rds'))
...
ggplot2::ggsave(plot=seventeen3, paste0('./figures/TechnicalValidation_current/CombinedFigMap3Panel_MismatchedPixels_', CDLYear, 'Only.pdf'),
                width=10, height=15)
```
