...
regionalextent <- c('Delaware'); regionName <- 'DE'
writetiles <- T
...
cdl_tiles <- beecoSp::grid_one_raster(rasterpath=cdl_path,
                            rasterID=paste0('CDL', CDLYear),
                            regionalextent=regionalextent, tiledir=tiledir,
                            div=div, buffercells=buffercells,
                            NAvalue=0, writetiles=writetiles)
tictoc::toc()

# run function to grid NE NVC into tiles (using parameters above)
nvc_tiles <- beecoSp::grid_one_raster(rasterpath=nvc_path, rasterID='NVC',
                            regionalextent=regionalextent, tiledir=tiledir,
                            div=div, buffercells=buffercells,
                            NAvalue=-9999, writetiles=writetiles)
...
# make list of arguements for mosaic function (terra rasters + 3 mosaic options)
