...
```{r loadtabulardata}
library(dplyr); library(logger); library(future)
...
```{r loadspatial}
state_rasters <- list.files('D:/MergeLANDFIRECDL_Rasters/2017', full.names = T)
...
all_freq <- furrr::future_map_dfr(.x=state_rasters, .f=tab_merged_raster,
