# function to regional or national raster into gridded tiles

grid_one_raster <- function(rpath, regionalextent=NA, rasterID,
  ...
  temp_raster <- raster::raster(rpath)
  ...
  if (!is.na(regionalextent)) {
    ...
    region_raster <- terra::rast(rpath) %>%
      terra::crop(y=region) %>%
