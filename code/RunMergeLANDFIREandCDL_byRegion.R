...
# note state or region name in log file
logger::log_info(paste0('Starting ', regionName, '.'))

# load shapefile for state/region 
regionalextent <- sf::st_read(paste0(datadir,'/SpatialData/', regionName , '_OnePoly.shp'))
...
for (i in 1:ceiling(end/30)) {
  ...
  if (length(get(paste0('args', i))) < 2) {
    ...
    assign(x=paste0('MT', i-1), value=rlang::exec("mosaic", !!!get(paste0('args', i-1)), fun='mean',
                                                  filename=paste0(tiledir, '/', regionName, '_CDLNVC8', i-1, '.tif'), overwrite=T))
  } else {
    ...
    assign(x=paste0('MT', i), value=rlang::exec("mosaic", !!!get(paste0('args', i)), fun='mean',
                                                filename=paste0(tiledir, '/', regionName, '_CDLNVCMerge', i, '.tif'), overwrite=T))
  }
}
...
if (length(mega_tiles) > 1 & length(mega_tiles) < 10) {
  wholemap <- rlang::exec("mosaic", !!!mega_tiles, fun='mean',
                          filename=paste0(tiledir, '/', regionName, '_FinalCDL', CDLYear,'NVCMerge.tif'), overwrite=T)
  # if only one mega-tile, just write this one
} else if (length(mega_tiles) == 1) {
  terra::writeRaster(MT1, filename=paste0(tiledir, '/', regionName, '_FinalCDL', CDLYear,'NVCMerge.tif'), overwrite=T) 
  # if there are lots of mega-tile (> 10), put some of these together before doing final merge
} else (length(mega_tiles) >= 10){
  ...
  for (i in 1:ceiling(end2/5)) {
    ...
    assign(x=paste0('M2T', i), value=rlang::exec("mosaic", !!!get(paste0('args2', i)), fun='mean',
                                                 filename=paste0(tiledir, '/', regionName, '_CDLNVCMergeM2T', i, '.tif'), overwrite=T))
  }
...
logger::log_info(paste0('Mosaic of ', regionName, ' rasters are complete!'))
