...
for (year in 2012:2020) {
  ...
  for (path in files) {
    ...
    toretain <- cbind(AL17_user, AL17_producer) %>%
      dplyr::mutate(State = substr(basename(path), start=10, stop=11), Year = substr(basename(path), start=12, stop=13)) %>%
...
write.csv(allstates, './data/CDL_Accuracy/CDL_accuracy_long_allstates_2012to2020.csv', row.names = F)
